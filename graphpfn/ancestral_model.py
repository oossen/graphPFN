...
class GraphPFNModel(NanoTabPFNModel):
    ...
    def _forward(self,
                ...
                prob_confounding_adj: torch.Tensor, 
                **kwargs) -> torch.Tensor:
        x_src, y_src = src
        ...
        output = self.transformer_encoder(input, single_eval_pos, prob_adj, prob_confounding_adj)
...
class TransformerEncoderStack(nn.Module):
    ...
    def forward(self, x: torch.Tensor, single_eval_position: int, prob_adj: torch.Tensor, prob_confounding_adj: torch.Tensor) -> torch.Tensor:
        """
        ...
        for block in self.transformer_blocks:
            x = block(x, single_eval_position=single_eval_position, prob_adj=prob_adj, prob_confounding_adj=prob_confounding_adj)
        return x
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, embedding_size: int, nhead: int, nhead_graph: int, mlp_hidden_size: int,
        ...
        self.self_attn_graph = MultiplicativeMultiheadAttention(embedding_size, 3 * nhead_graph) # 4 parent heads, 4 child heads, 4 confounding heads
    ...
    def forward(self, src: torch.Tensor, single_eval_position: int, prob_adj: torch.Tensor, prob_confounding_adj: torch.Tensor) -> torch.Tensor:
        """
        ...
        mask = calculate_mask(prob_adj, prob_confounding_adj, self.nhead_graph)
