...
class GraphPFNModel(nn.Module):
    ...
    def _forward(self, src: Tuple[torch.Tensor, torch.Tensor], single_eval_pos: int, num_mem_chunks: int = 1, **kwargs) -> torch.Tensor:
        x_src, y_src = src
        ...
        # Note: If C is constant, you should ideally pre-compute this in __init__
        pe = get_positional_encoding(C, E, src.device)
        
        # Add positional encoding to src
        # The (1, 1, C, E) shape broadcasts automatically to (B, R, C, E)
        src = src + pe
        # ------------------------------------
...
def get_positional_encoding(c_dim: int, e_dim: int, device: torch.device):
    """
    Standard sinusoidal positional encoding.
    Returns a tensor of shape (1, 1, C, E)
    """
    pe = torch.zeros(c_dim, e_dim, device=device)
    position = torch.arange(0, c_dim, dtype=torch.float, device=device).unsqueeze(1)
    div_term = torch.exp(torch.arange(0, e_dim, 2, dtype=torch.float, device=device) * 
                         (-math.log(10000.0) / e_dim))
    
    pe[:, 0::2] = torch.sin(position * div_term)
    pe[:, 1::2] = torch.cos(position * div_term)
    
    # Shape it as (1, 1, C, E) to allow broadcasting across Batches and Rows
