...
class Regressor(NanoTabPFNRegressor):
    ...
    def predict(self, X_test: np.ndarray, **kwargs):
        ...
        with torch.no_grad():
            ...
            preds = preds_n * torch.tensor(self.y_train_std, dtype=torch.float32, device=self.device) + torch.tensor(self.y_train_mean, dtype=torch.float32, device=self.device)
