...
class Regressor(NanoTabPFNRegressor):
    ...
    def fit(self, X_train: ndarray, y_train: ndarray):
        ...
        """
        self.X_train_tensor = torch.tensor(X_train, dtype=torch.float32, device=self.device).unsqueeze(0) # now shape (1, N, F)
        self.y_train_tensor = torch.tensor(y_train, dtype=torch.float32, device=self.device).unsqueeze(0) # now shape (1, N)
        self.X_train_tensor, self.y_train_tensor = self.preprocessor.fit(self.X_train_tensor, self.y_train_tensor) # same shapes still
    ...
    def predict(self, X_test: ndarray) -> ndarray:
        ...
        """
        X_test_tensor = torch.tensor(X_test, dtype=torch.float32, device=self.device).unsqueeze(0) # now shape (1, N, F)
        X_test_tensor = self.preprocessor.process_x(torch.Tensor(X_test_tensor)) # same shape still
        X_tensor = torch.concatenate((self.X_train_tensor, X_test_tensor), dim=1)
        single_eval_pos = self.X_train_tensor.shape[1]

        with torch.no_grad():
            logits = self.model((X_tensor, self.y_train_tensor), single_eval_pos=single_eval_pos) # (1, N, n_buckets)
            preds = self.dist.mean(logits) # (1, N)
