import importlib.util
from datetime import datetime
import os
import shutil

from prior.dataloaders.observational_dataloader import ObservationalDataLoader
from visualization.check_seeding import compare_dataloaders
from visualization.plotting import plot_r2, plot_correlation, plot_graph, plot_point_clouds


def make_all(config: str, n_steps: int = 10, check_seeding=True, include_r2=True):
    """
    Run the complete visualization suite for the prior specified by `config`.
    This must be a string specifying a path to a configuration file.
    """
    spec = importlib.util.spec_from_file_location("config_module", config)
    assert spec is not None and spec.loader is not None, "Invalid config!"
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    
    prior_config = module.prior_config
    preprocessing_config = module.preprocessing_config
    
    if check_seeding:
        prior_1 = ObservationalDataLoader(n_steps, 5, prior_config, preprocessing_config, 42)
        prior_2 = ObservationalDataLoader(n_steps, 5, prior_config, preprocessing_config, 42)
        compare_dataloaders(prior_1, prior_2)
    
    
    now = datetime.now()
    datetime_str = now.strftime("%m_%d_%H_%M")
    config_name = config.split('/')[-1][:-3] # remove .py
    output_path = f"visualization/output/{config_name}_{datetime_str}"
    os.makedirs(output_path, exist_ok=True)
    
    # copy config file
    shutil.copy(config, f"{output_path}/configs.py")
    
    if include_r2:
        big_prior = ObservationalDataLoader(10 * n_steps, 1, prior_config, preprocessing_config, 42)
        plot_r2(big_prior, f"{output_path}/r2.png")
    
    prior = ObservationalDataLoader(n_steps, 1, prior_config, preprocessing_config, 42)
    for i, data in enumerate(prior):
        X = data['x'][0]
        g = data['graph']
        plot_graph(g, f"{output_path}/graph_{i}.png")
        plot_correlation(X, f"{output_path}/correlation_{i}.png")
        plot_point_clouds(X, f"{output_path}/point_clouds_{i}.png")
        
        # write sampled parameters to file
        with open(f"{output_path}/sampled_params_{i}.py", "w") as f:
            for name, param_dict in data["sampled_params"].items():
                f.write(f"{name} = {repr(param_dict)}\n")
    

if __name__ == "__main__":
    prior = "prior/configs/default_configs.py"
    make_all(prior)